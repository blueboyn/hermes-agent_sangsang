"""Unified store: the single place where *declarative knowledge* (agi-style)
and *procedural skills* (hermes-style) live side by side, with provenance.

This is the structural fusion. agi kept knowledge in MySQL+Neo4j; hermes kept
skills in Markdown files. Chiron keeps both in one SQLite database so that the
*promotion* step (knowledge -> skill) is a local transaction, not an ETL job.

Design rules carried over from the two parents:
  * hermes: never delete — archive. Every write carries provenance.
  * agi: edges have a lifecycle status (hypothesis -> validated / rejected).
"""
...
CREATE TABLE IF NOT EXISTS edges (
    ...
    status TEXT NOT NULL DEFAULT 'hypothesis',   -- hypothesis|validated|rejected|doubtful
    origin TEXT NOT NULL DEFAULT 'acquired',      -- acquired|emergent
...
CREATE TABLE IF NOT EXISTS goals (
    id INTEGER PRIMARY KEY,
    gtype TEXT NOT NULL,            -- KNOWLEDGE_GAP|CAPABILITY_FIX
...
CREATE TABLE IF NOT EXISTS skills (
    ...
    name TEXT UNIQUE NOT NULL,
    body TEXT NOT NULL,
    origin TEXT NOT NULL,           -- foreground|background_review|promoted|curated
...
CREATE TABLE IF NOT EXISTS validations (
    id INTEGER PRIMARY KEY,
    target_type TEXT NOT NULL,      -- edge|promotion
